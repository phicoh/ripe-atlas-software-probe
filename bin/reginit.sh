...
if [ "$need_rereg" == 1 ]; then 
	...
	echo "$SSH_CMD -i $SSH_PVT_KEY -p 443 atlas@$R_HOST INIT"
	$SET_LEDS_CMD reg-init
	$P_TO_R_INIT_CMD $R | $SSH_CMD $SSH_OPT -i $SSH_PVT_KEY -p 443 atlas@$R_HOST INIT > $REG_INIT_REPLY 2>$SSH_ERR
	ERR=$?                                                            
	if [ $ERR != "0" ] ; then                                         
		D=`epoch`
...
then
	...
	if [ $FIRMWARE_APPS_VERSION -gt  $FIRMWARE_APPS_VERSION_MY ] ; then
		...
		$SSH_CMD $SSH_OPT -i $SSH_PVT_KEY -p $CONTROLLER_1_PORT atlas@$CONTROLLER_1_HOST  "FIRMWARE_APPS $FIRMWARE_APPS" > $FIRMWARE_FETCH_DIR/$FIRMWARE_APPS 2>$SSH_ERR
		ERR=$?
	        if [ $ERR != "0" ] ; then
			D=`epoch`
...
## Controller  INIT 
	...
	$SSH_CMD $SSH_OPT -p $CONTROLLER_1_PORT -i $SSH_PVT_KEY atlas@$CONTROLLER_1_HOST INIT > $CON_INIT_REPLY 2>$SSH_ERR
	ERR=$?                                                            
	if [ $ERR != "0" ] ; then                                         
		D=`epoch`
...
$SSH_CMD_EXEC $SSH_OPT -R $REMOTE_PORT:127.0.0.1:$TELNETD_PORT -L 8080:127.0.0.1:8080 -i $SSH_PVT_KEY -p $CONTROLLER_1_PORT atlas@$CONTROLLER_1_HOST KEEP > $CON_KEEP_REPLY 2>$SSH_ERR &
KEEP_PID=$!
